...
generator.modify_preset({'min_freq':40, 'max_freq':1600})
...
fname = f'DataCubes/CasA_timeint.fits'
...
for f in glob.glob(fname):
    ...
    wlens = np.linspace(8.321e-4, 2.101e-4, data.shape[0])
    # # data = data[2100:2230]
    # data = data[700:900]
    ...
    for i in range(censel.shape[1]):
        for j in range(censel.shape[2]):
            ...
            soni.render()
            print(vols[i,j])
            soni.save(f'static/audio/snd_{i}_{j}.wav', master_volume=vols[i,j]**1.15)
    ...
    outspec = (spec.T/spec.max())
    outspec = np.log10(np.clip(outspec, 1e-4,1))
